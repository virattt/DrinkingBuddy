...
public class DrinkUpdateReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		sharedPrefs = context.getSharedPreferences(SettingsFragment.SHARED_PREFS,
				Context.MODE_PRIVATE);
		mSoundsPrefs = sharedPrefs.getBoolean(SettingsFragment.SOUNDS_PREFS, true);
		mVibrationPrefs = sharedPrefs.getBoolean(SettingsFragment.VIBRATION_PREFS, true);
		...
		if (DayLab.get(context).getDrinkLab(drinkLabId) != null) {
			...
			inboxStyle.addLine("Estimate BAC: " + BAC + "%");
			...
			Notification.Builder notificationBuilder = new Notification.Builder(
					...
					.setContentIntent(mContentIntent).setStyle(inboxStyle);
